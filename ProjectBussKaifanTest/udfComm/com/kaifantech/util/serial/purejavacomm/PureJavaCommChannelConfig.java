package com.kaifantech.util.serial.purejavacomm;
 ...
 * {@link DefaultPureJavaCommChannelConfig} allows the following options in the
 * option map:
 *
 ...
 * </tr>
 * <tr>
 * <td>{@link PureJavaCommChannelOption#BAUD_RATE}</td>
 * <td>{@link #setBaudrate(int)}</td>
 * </tr>
 * <tr>
 * <td>{@link PureJavaCommChannelOption#DTR}</td>
 * <td>{@link #setDtr(boolean)}</td>
 * </tr>
 * <tr>
 * <td>{@link PureJavaCommChannelOption#RTS}</td>
 * <td>{@link #setRts(boolean)}</td>
 * </tr>
 * <tr>
 * <td>{@link PureJavaCommChannelOption#STOP_BITS}</td>
 * <td>{@link #setStopbits(Stopbits)}</td>
 * </tr>
 * <tr>
 * <td>{@link PureJavaCommChannelOption#DATA_BITS}</td>
 * <td>{@link #setDatabits(Databits)}</td>
 * </tr>
 * <tr>
 * <td>{@link PureJavaCommChannelOption#PARITY_BIT}</td>
 * <td>{@link #setParitybit(Paritybit)}</td>
 * </tr>
 * <tr>
 * <td>{@link PureJavaCommChannelOption#WAIT_TIME}</td>
 * <td>{@link #setWaitTimeMillis(int)}</td>
 * </tr>
 * </table>
 */
public interface PureJavaCommChannelConfig extends ChannelConfig {
	...
	enum Paritybit {
		...
		/**
		 * An odd parity bit will be sent with each data character, ie. will be
		 * set to 1 if the data character contains an even number of bits set to
		 * 1.
		 */
		ODD(SerialPort.PARITY_ODD),
		/**
		 * An even parity bit will be sent with each data character, ie. will be
		 * set to 1 if the data character contains an odd number of bits set to
		 * 1.
		 */
		EVEN(SerialPort.PARITY_EVEN),
		/**
		 * A mark parity bit (ie. always 1) will be sent with each data
		 * character
		 */
		MARK(SerialPort.PARITY_MARK),
		/**
		 * A space parity bit (ie. always 0) will be sent with each data
		 * character
		 */
	...
	/**
	 * Sets the baud rate (ie. bits per second) for communication with the
	 * serial device. The baud rate will include bits for framing (in the form
	 * of stop bits and parity), such that the effective data rate will be lower
	 * than this value.
	 *
	 * @param baudrate
	...
	/**
	 * Sets the number of stop bits to include at the end of every character to
	 * aid the serial device in synchronising with the data.
	 *
	 * @param stopbits
	...
	/**
	 * Sets the type of parity bit to be used when communicating with the serial
	 * device.
	 *
	 * @param paritybit
	...
	/**
	 * @return The configured stop bits, defaulting to
	 *         {@link Stopbits#STOPBITS_1} if unset
	 */
	Stopbits getStopbits();

	/**
	 * @return The configured data bits, defaulting to
	 *         {@link Databits#DATABITS_8} if unset
	 */
	Databits getDatabits();

	/**
	 * @return The configured parity bit, defaulting to {@link Paritybit#NONE}
	 *         if unset
	 */
	Paritybit getParitybit();

	/**
	 * @return true if the serial device should support the Data Terminal Ready
	 *         signal
	 */
	boolean isDtr();

	/**
	 * Sets whether the serial device supports the Data Terminal Ready signal,
	...
	/**
	 * Sets whether the serial device supports the Request To Send signal, used
	 * for flow control
	 *
	 * @param rts
	...
	/**
	 * @return The number of milliseconds to wait between opening the serial
	 *         port and initialising.
	 */
	int getWaitTimeMillis();

	/**
	 * Sets the time to wait after opening the serial port and before sending it
	 * any configuration information or data. A value of 0 indicates that no
	 * waiting should occur.
	 *
	 * @param waitTimeMillis
	 *            The number of milliseconds to wait, defaulting to 0 (no wait)
	 *            if unset
	...
	/**
	 * Sets the maximal time (in ms) to block while try to read from the serial
	 * port. Default is 1000ms
	 */
	PureJavaCommChannelConfig setReadTimeout(int readTimout);

	/**
	 * Return the maximal time (in ms) to block and wait for something to be
	 * ready to read.
	 */
	int getReadTimeout();
